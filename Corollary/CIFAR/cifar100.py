...
Xtr,Ytr,Xte,Yte=load_cifar100("./datasets/cifar100/")
...
from sklearn.tree import DecisionTreeClassifier
memory_start1=psutil.Process(os.getpid()).memory_info().rss/1024 #KB
start_time_base1=time.time()
classifier1=DecisionTreeClassifier(criterion='gini',splitter='best',max_depth=5,min_samples_split=5,random_state=42)#第一次测试最优
...
print('基分类器1(5)所用时间:',base_time1)
memory_end1 =psutil.Process(os.getpid()).memory_info().rss/1024  #KB
memory_usage1=memory_end1-memory_start1
print('基分类器1(5)所用内存(KB):',memory_usage1)

import joblib

#保存Model
joblib.dump(classifier1,'model_cifar100/base1_learner_cifar100.pkl')
print("基分类器1pkl_size(MB):",os.path.getsize('model_cifar100/base1_learner_cifar100.pkl')/1024/1024) #MB
...
def bagging_clf(base_learner,target_dir,n):
    ...
    for i in tqdm(n):
        ...
        memory_start=psutil.Process(os.getpid()).memory_info().rss/1024 #KB
    ...
    #     print('Bagging所用时间：',bagging_time)
        ...
        memory_end  =psutil.Process(os.getpid()).memory_info().rss/1024  #KB
...
time_bagging1,top1_bagging1,m_percent_bagging1=bagging_clf(classifier1,'model_cifar100/model1_bagging',n)
...
time_boosting1,top1_boosting1,m_percent_boosting1=boosting_clf(classifier1,'model_cifar100/model1_boosting',n)
...
#基分类器2
memory_start2=psutil.Process(os.getpid()).memory_info().rss/1024 #KB
...
memory_end2 =psutil.Process(os.getpid()).memory_info().rss/1024  #KB
memory_usage2=memory_end2-memory_start2
print('基分类器2(15)所用内存(KB):',memory_usage2)
#保存Model
joblib.dump(classifier2,'model_cifar100/base2_learner_cifar100.pkl')
print("基分类器2pkl_size(MB):",os.path.getsize('model_cifar100/base2_learner_cifar100.pkl')/1024/1024)

#Bagging2
time_bagging2,top1_bagging2,m_percent_bagging2=bagging_clf(classifier2,'model_cifar100/model2_bagging',n)
...
#Boosting2
time_boosting2,top1_boosting2,m_percent_boosting2=boosting_clf(classifier2,'model_cifar100/model2_boosting',n)
...
#基分类器3
memory_start3=psutil.Process(os.getpid()).memory_info().rss/1024 #KB
...
memory_end3 =psutil.Process(os.getpid()).memory_info().rss/1024  #KB
memory_usage3=memory_end3-memory_start3
print('基分类器3(20)所用内存(KB):',memory_usage3)
#保存Model
joblib.dump(classifier3,'model_cifar100/base3_learner_cifar100.pkl')
print("基分类器3pkl_size(MB):",os.path.getsize('model_cifar100/base3_learner_cifar100.pkl')/1024/1024)

# Bagging3
time_bagging3,top1_bagging3,m_percent_bagging3=bagging_clf(classifier3,'model_cifar100/model3_bagging',n)
...
# Boosting3
time_boosting3,top1_boosting3,m_percent_boosting3=boosting_clf(classifier3,'model_cifar100/model3_boosting',n)
...
#基分类器4
memory_start4=psutil.Process(os.getpid()).memory_info().rss/1024 #KB
...
memory_end4 =psutil.Process(os.getpid()).memory_info().rss/1024  #KB
memory_usage4=memory_end4-memory_start4
print('基分类器4(10)所用内存(KB):',memory_usage4)
#保存Model
joblib.dump(classifier4,'model_cifar100/base4_learner_cifar10.pkl')
print("基分类器4pkl_size(MB):",os.path.getsize('model_cifar100/base4_learner_cifar10.pkl')/1024/1024)

# Bagging4
time_bagging4,top1_bagging4,m_percent_bagging4=bagging_clf(classifier4,'model_cifar100/model4_bagging',n)
pkl_size_bagging4=get_dir_size('model_cifar100/model4_bagging')
#保存结果
import pandas as pd
c4={"基分类器个数" : n,
...
bagging_result4.to_csv('model_cifar100/bagging_cifar100_result4.csv')

# Boosting4
time_boosting4,top1_boosting4,m_percent_boosting4=boosting_clf(classifier4,'model_cifar100/model4_boosting',n)
pkl_size_boosting4=get_dir_size('model_cifar100/model4_boosting')
#保存结果
import pandas as pd
cb4={"基分类器个数" : n,
...
print(boost_result4)
boost_result4.to_csv('model_cifar100/boosting_cifar100_result4.csv')